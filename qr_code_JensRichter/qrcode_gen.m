...
% ---------------
% Syntax:
%   qrcode_gen(message, 'QuietZone',12);
%
% Description:
%   Defines the size of the quiet zone. Even a quiet zone setting of 0 will
%   generate a minimal quietzone. Effectively the qr codes upper left corner 
%   will have the coordinates x = QuietZoneValue and y = QuietZoneValue if 
%   the QuietZone setting is bigger than the minimal quietzone.
%
%
...
if(successloading)
    ...
    try
        ...
        % For defining the margin or quiet zone
        qr_hints.put(com.google.zxing.EncodeHintType.MARGIN,  java.lang.Integer.valueOf(qrcodeProps.setting.quietZoneSize));
        ...
        warning('An Error occured during generating the QR-Code - This version of qrcode_gen is optimized for zxing 3.2.0 and can be used with Matlab2014a or newer releaes.');
        qr = 0;
    end
    ...
    function validateUserArguments(vararginRaw)
        ...
        % Check Quiet Zone
        checkQuietZoneInput(vararginRaw)
    ...
    function checkSpecialCommandInput(vararginStruct)
        ...
        if ~isempty(out.value)
            try
                ...
                warning('The parameter DownloadJars needs to have a numerical value of 0 (no dowload) or 1 (download)');
            end
        ...
        try
            ...
            for jj=1:numel(qrcodeProps.jar.files)
                ...
                if matlabVersionChecker('2014b')
                    websave(saveToFileName,curDownloadPath); % This feature is only available from Matlab 2014b on
                else
    ...
    function checkQRCodeSizeInput(vararginStruct)
        ...
        if qrcodeProps.setting.qrverUserAssigned
            ...
            temp = qrcodeProps.setting.initialSize + 4*qrcodeProps.setting.qrver;
            qrcodeProps.setting.size = [temp,temp];
    ...
    function checkQuietZoneInput(vararginStruct)
        
        out = deleteFromArgumentAndGetValue(vararginStruct,'QuietZone');
        if ~isempty(out.value)
            if isnumeric(out.value)
                %User setting is valid
                qrcodeProps.setting.quietZoneSize = out.value;
            else
                warning('The quiet zone setting needs to be a number - the default value is 4');
            end
        end
...
%% This function will help us do identify if the current Matlab version is higher or equal to the minversion we need for certain feature.
